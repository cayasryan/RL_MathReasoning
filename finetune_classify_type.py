...
wandb.init(project="MATH-finetuning", name="bert_type_classification")
...
train_df["type_id"] = type_encoder.fit_transform(train_df["type"])
val_df["type_id"] = type_encoder.transform(val_df["type"])  # use same encoder!
...
train_dataset = tokenize_and_format(train_df, "type_id")
val_dataset = tokenize_and_format(val_df, "type_id")

# === Model & Trainer ===
model = BertForSequenceClassification.from_pretrained(model_name, num_labels=len(type_encoder.classes_))

training_args = TrainingArguments(
    output_dir="./bert_type",
    ...
    run_name="bert_type_classification", 
)
